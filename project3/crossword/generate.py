...
class CrosswordCreator():
    ...
    def ac3(self, arcs=None):
        ...
        """
        if arcs is None:
            queue = collections.deque((x, y) for x in self.crossword.variables for y in self.crossword.neighbors(x))
        else:
    ...
    def select_unassigned_variable(self, assignment):
        ...
        def num_remaining_values(var):
            return len(self.domains[var])

        def degree(var):
            return len(self.crossword.neighbors(var))

        unassigned_vars.sort(key=lambda var:(num_remaining_values(var), -degree(var)))
